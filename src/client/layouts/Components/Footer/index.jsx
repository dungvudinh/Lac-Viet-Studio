            ...
            <Box width="100%" bgcolor="#fff" height={40} paddingLeft={2} borderRadius={1} display="flex" flexDirection='row' justifyContent='space-between'>
              ...
              <Button variant="contained" 
                sx={{
                  ...
                  fontSize:{ xs:'var(--fs-md)' }
                }}>
                  Sign Up
              </Button>
            </Box>  
            <Typography variant="p" component="p" fontSize={'var(--fs-sm)'} marginTop={1}>
        ...
        <Box pb="10px" borderTop="1px solid #616161" borderBottom="1px solid #616161">
          <Box sx={{ display: { xs: 'block', sm: 'none' } }}>
            <Accordion sx={{ 
              ...
              >
                <Typography sx={{ fontSize:'var(--fs-md)' }} varient={'h6'}>Product</Typography>
              </AccordionSummary>
              <AccordionDetails>
                {PRODUCT_ITEMS.map((item, index) => (
                  <Typography key={index} sx={{ fontSize:'var(--fs-md)', padding:'0.3rem 0' }}>{item.title}</Typography>
                ))}
              </AccordionDetails>
            </Accordion>
            ...
            <Accordion sx={{ 
              ...
              <AccordionSummary 
                ...
                sx={{ padding:'0 !important' }}
              >
                <Typography sx={{ fontSize:'var(--fs-md)' }}>Company</Typography>
              </AccordionSummary>
              <AccordionDetails>
                {COMPANY_ITEMS.map((item, index) => (
                  <Typography key={index} sx={{ fontSize:'var(--fs-md)', padding:'0.4rem 0' }}>{item.title}</Typography>
                ))}
              </AccordionDetails>
            </Accordion>
          </Box>
          ...
          <Grid container position="relative" direction={{ xs:'column', md:'row' }} justifyContent={'space-between'}>
            ...
            <Grid xs={12} md={2} textAlign={{ xs: 'left', md: 'right' }} mt={{ xs: 2, md: 0 }} sx={{ display:'flex', flexDirection:'row' }}>
              <Link underline="none" href="" mr="1rem">
